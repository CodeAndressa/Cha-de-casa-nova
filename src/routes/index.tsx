...
import { MapPin, Calendar, Clock, MessageCircle, Copy, Heart, Gift as GiftIcon, Check, Sparkles } from "lucide-react";
...
function PublicPage() {
  ...
  return (
    <div className="relative min-h-screen isolate overflow-hidden bg-background text-foreground">
      ...
      <div className="pointer-events-none fixed inset-0 z-[1] bg-[radial-gradient(circle_at_20%_10%,oklch(0.92_0.05_115_/_0.58),transparent_34%),linear-gradient(180deg,oklch(0.98_0.018_118_/_0.94),oklch(0.94_0.025_122_/_0.9)_48%,oklch(0.98_0.012_105_/_0.96))]" />
      ...
      <main className="relative z-10">
        <header className="mx-auto flex min-h-screen max-w-7xl flex-col px-5 py-5 sm:px-8 lg:px-10">
          <nav className="flex items-center justify-between rounded-lg border border-white/60 bg-white/55 px-4 py-3 shadow-premium backdrop-blur-xl">
            <a href="#top" className="font-display text-xl text-forest">Chá de Casa Nova</a>
            <div className="hidden items-center gap-6 text-xs font-medium uppercase tracking-[0.22em] text-foreground/65 sm:flex">
              <a href="#presentes" className="transition hover:text-forest">Presentes</a>
              <a href="#pix" className="transition hover:text-forest">Pix</a>
              <a href="#recado" className="transition hover:text-forest">Recado</a>
            </div>
          ...
          <section id="top" className="grid flex-1 items-center gap-10 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:py-10">
            <div className="max-w-3xl">
              ...
              <p className="mt-6 max-w-2xl text-base leading-8 text-foreground/72 sm:text-lg">
                {event?.welcome_text ?? "Uma tarde para celebrar o novo lar, dividir histórias boas e receber quem faz parte da nossa vida."}
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-3 text-sm text-foreground/75">
                ...
                {event?.city && (
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/65 bg-white/55 px-4 py-2 shadow-soft backdrop-blur">
                    <MapPin className="h-4 w-4 text-terracotta" /> {event.city}{event.state ? ` - ${event.state}` : ""}
                  </span>
                )}
              </div>

              <div className="mt-10 flex flex-wrap items-center gap-3">
                <Button size="lg" className="rounded-lg px-7 shadow-premium" onClick={() => setRsvpOpen(true)}>
                  Confirmar presença
                </Button>
                <Button asChild size="lg" variant="outline" className="rounded-lg border-white/70 bg-white/55 px-7 backdrop-blur">
                  <a href="#presentes"><GiftIcon className="mr-2 h-4 w-4" /> Ver lista</a>
                </Button>
              </div>
            </div>
              ...
              {cd && !cd.finished && (
                <div className="mt-4 grid grid-cols-4 gap-2">
                  {[
                    ...
                    <div key={x.label} className="rounded-lg border border-forest/10 bg-card/78 px-2 py-3 text-center shadow-soft backdrop-blur">
                      <div className="font-display text-3xl text-forest">{String(x.v).padStart(2, "0")}</div>
          ...
          <section className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
            <Card className="rounded-lg border-white/70 bg-white/62 p-5 shadow-premium backdrop-blur-xl sm:p-7">
              <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button asChild variant="outline" className="rounded-lg border-forest/15 bg-white/60">
        ...
        <section id="presentes" className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:py-20">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-3 font-display text-5xl text-forest sm:text-6xl">Lista de presentes</h2>
          ...
          <section id="pix" className="px-5 py-16 sm:px-8">
            <div className="mx-auto max-w-4xl rounded-lg border border-white/70 bg-forest/92 p-6 text-forest-foreground shadow-premium backdrop-blur-xl sm:p-8">
              <div className="grid gap-7 md:grid-cols-[0.9fr_1.1fr] md:items-center">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-white/62">Contribuir via Pix</p>
                  <h2 className="mt-3 font-display text-4xl text-white sm:text-5xl">Um mimo para o novo lar</h2>
                  <p className="mt-4 text-sm leading-7 text-white/70">Se preferir, você também pode contribuir por Pix de forma simples.</p>
                </div>
                <div className="grid gap-5 sm:grid-cols-[auto_1fr] sm:items-center">
                  {event?.pix_qr_url && (
                    <img src={event.pix_qr_url} alt="QR Code Pix" className="mx-auto h-40 w-40 rounded-lg border border-white/20 bg-white p-2" />
                  )}
                    ...
                    {event?.pix_key && (
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <code className="flex-1 truncate rounded-lg border border-white/15 bg-white/10 px-3 py-2 text-sm text-white">{event.pix_key}</code>
                          <Button size="icon" variant="secondary" onClick={copyPix} aria-label="Copiar chave Pix" className="rounded-lg">
...
function FloatingPhotoBackground({ photos, fallback }: { photos: CouplePhoto[]; fallback: string }) {
  const backgroundPhotos = photos.length > 0 ? photos : [{ id: "fallback", image_url: fallback, caption: null }];
  ...
  return (
    <button
      ...
      className={`rounded-full border px-4 py-1.5 text-sm font-medium transition-all ${
        active ? "border-forest bg-forest text-forest-foreground shadow-soft" : "border-white/70 bg-white/58 text-foreground/72 backdrop-blur hover:border-forest/30 hover:text-forest"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <Card className="group overflow-hidden rounded-lg border-white/70 bg-white/68 p-0 shadow-soft backdrop-blur-xl transition-all hover:-translate-y-1 hover:shadow-premium">
      <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
        {gift.image_url ? (
          <img src={gift.image_url} alt={gift.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
        ) : (
      ...
      <div className="p-5">
        ...
        {gift.description && <p className="mt-2 line-clamp-2 text-sm leading-6 text-muted-foreground">{gift.description}</p>}
        <div className="mt-5 flex items-center justify-between gap-3">
          <span className="text-sm font-semibold text-foreground">{formatBRL(gift.estimated_value)}</span>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setStatus("reservado")} className={`rounded-lg border p-3 text-sm transition-all ${status === "reservado" ? "border-primary bg-secondary" : "border-border"}`}>
                Reservar (vou levar)
              </button>
              <button type="button" onClick={() => setStatus("presenteado")} className={`rounded-lg border p-3 text-sm transition-all ${status === "presenteado" ? "border-primary bg-secondary" : "border-border"}`}>
                Já presenteado
              </button>
            </div>
          </div>
  ...
  return (
    <section id="recado" className="mx-auto max-w-3xl px-5 py-16 sm:px-8">
      <div className="text-center">
        ...
        <h2 className="mt-3 font-display text-4xl text-forest sm:text-5xl">Um recado para os novos moradores</h2>
      </div>
      <Card className="mt-8 rounded-lg border-white/70 bg-white/66 p-5 shadow-premium backdrop-blur-xl sm:p-7">
        <div className="space-y-4">
          ...
          <Button className="w-full rounded-lg" onClick={() => mutation.mutate()} disabled={!name.trim() || !message.trim() || mutation.isPending}>
            Enviar mensagem
          </Button>
        </div>
      </Card>
